...
describe Kafka::Broker do
  ...
  describe "#produce" do
    ...
    it "gives me a console if a partition is in error" do
      ...
      topic_info = Kafka::Protocol::ProduceResponse::TopicInfo.new(topic: "articles", partitions: [1])
      allow(response).to receive(:each_partition) { [[topic_info, errored_partition_info]] }
      connection.mock_response(response)
